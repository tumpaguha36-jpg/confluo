...
/* ---------------- Connection badge ---------------- */
function ConnectionBadge({ state, unsynced }) {
  const map = {
    connecting: { icon: RefreshCw, cls: 'text-muted-foreground', label: 'Connecting…', spin: true },
    connected: { icon: Wifi, cls: 'text-success', label: 'Synced' },
    reconnecting: { icon: RefreshCw, cls: 'text-warning', label: 'Reconnecting…', spin: true },
    offline: { icon: WifiOff, cls: 'text-muted-foreground', label: 'Changes saved on this device' },
    denied: { icon: Ban, cls: 'text-destructive', label: 'Access denied' },
  };
  const c = map[state] || map.connected;
  const Icon = c.icon;
  return (
    <div className="flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1 text-2xs font-medium" aria-live="polite">
      <Icon className={`h-3.5 w-3.5 ${c.cls} ${c.spin ? 'animate-spin' : ''}`} />
      <span className={c.cls}>{c.label}</span>
      {unsynced && state === 'connected' && <span className="ml-0.5 h-1.5 w-1.5 rounded-full bg-warning" title="Unsynced changes" />}
    </div>
  );
}
        ...
        {shown.map((p, i) => (
          <Tooltip key={p.id + i}>
            <TooltipTrigger asChild>
              <span
                className={`grid h-8 w-8 place-items-center rounded-full border-2 border-background text-[12px] font-semibold text-white transition-opacity ${p.idle ? 'opacity-40' : ''}`}
                style={{ background: p.color }}
              >
                {(p.name || '?')[0].toUpperCase()}
              </span>
            </TooltipTrigger>
            <TooltipContent>{p.name}{p.self ? ' (you)' : ''} · <span className="capitalize">{p.role}</span>{p.idle ? ' · idle' : ''}</TooltipContent>
          </Tooltip>
        ))}
...
export default function EditorPage() {
  ...
  const editable = doc ? canEdit(role) : false;
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      contentTimer.current = setTimeout(() => saveContent(editor.getJSON()), 1000);
    },
  ...
  const saveContent = useCallback(async (json) => {
    try {
      ...
      setSaveState('saved'); setUnsynced(false); setConn('connected');
    } catch (e) {
      setSaveState('error'); setConn('offline');
    }
  }, [id]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setDoc(doc); setRole(doc.role); setTitle(doc.title);
        versionRef.current = doc.version || 1;
        // simulated presence: other collaborators as idle peers
        const others = (doc.collaborators || []).filter((c) => c.id !== user.id).slice(0, 4).map((c, i) => ({ ...c, idle: i % 2 === 1 }));
        setPeers(others);
      } catch (e) {
  ...
  useEffect(() => {
    setConn('connecting');
    const t = setTimeout(() => setConn('connected'), 700);
    return () => clearTimeout(t);
  }, [id]);

  // keep editor content synced once loaded
  useEffect(() => {
    if (editor && doc) {
  ...
  const onTitleChange = (v) => {
    ...
    titleTimer.current = setTimeout(async () => {
      try {
        ...
        if (e.status === 412) { toast.warning('Renamed elsewhere — adopting the server title'); setTitleState('error'); }
  ...
  // leave protection
  useEffect(() => {
    const onBefore = (e) => { if (unsynced || uploadsPending.current > 0) { e.preventDefault(); e.returnValue = ''; } };
  ...
  const insertImageFile = async (file) => {
    ...
    reader.onload = () => {
      // Simulated upload latency; never blocks typing.
      setTimeout(() => {
  ...
  const me = { id: user.id, name: user.name, color: user.color, role, self: true };
    ...
    <div className="flex h-screen flex-col bg-background">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-border px-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <ConnectionBadge state={conn} unsynced={unsynced} />
          <ShareDialog doc={doc} role={role} me={user} onUpdated={(c) => setPeers(c.filter((x) => x.id !== user.id))} />
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="relative flex-1 overflow-y-auto">
            ...
            {/* floating comment button on selection for commenters (read-only) */}
            {readOnly && canComment(role) && selection && (
              <div className="pointer-events-auto fixed bottom-6 left-1/2 z-30 -translate-x-1/2">
                <Button size="sm" onClick={() => setSidebarOpen(true)}><MessageSquare className="mr-1.5 h-4 w-4" /> Comment on selection</Button>
              </div>
            )}
          </div>
        </div>
